...
/* ─── Campaign Banner — single current campaign, no carousel ───────────── */
function CampaignBanner({ campaign, navigate }) {
  ...
  return (
    <div style={{ marginBottom: 28, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10 }}>
      <div
        onClick={goToOffer}
        title={`View ${campaign.name} & buy`}
        style={{
          width: '100%', maxWidth: 900, cursor: 'pointer',
          borderRadius: 14, overflow: 'hidden', border: '1.5px solid #c7d2fe',
          boxShadow: '0 4px 20px rgba(79,70,229,.14)',
          transition: 'transform .18s, box-shadow .18s',
        }}
        onMouseEnter={e => { e.currentTarget.style.transform = 'translateY(-3px)'; e.currentTarget.style.boxShadow = '0 14px 34px rgba(79,70,229,.24)' }}
        onMouseLeave={e => { e.currentTarget.style.transform = 'none'; e.currentTarget.style.boxShadow = '0 4px 20px rgba(79,70,229,.14)' }}
      >
        <img src={campaign.campaignImage} alt={campaign.name}
          style={{ width: '100%', height: 'auto', display: 'block' }} />
      </div>
      <div style={{ fontSize: 12, color: '#6b7fa0' }}>Tap the banner to view this offer and buy</div>
    </div>
  )
}
...
export default function MemberPortal() {
  ...
  const myMemberRecord = members.find(m => m.email === user?.email)
  const [promoCampaigns] = useState(() => getActivePromoCampaigns(myMemberRecord?.associationId))
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
      ...
      {promoCampaigns.length > 0
        ? <CampaignBanner campaign={promoCampaigns[0]} navigate={navigate} />
              ...
              return (
                <div key={i} style={{ border:`1px solid ${urgent?'#fde68a':'#e8e4f0'}`, borderRadius:10, padding:'14px 16px', marginBottom:10, background: urgent?'#fffbeb':'#faf9fc' }}>
                  ...
                  <div className="cp-pol-details" style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:10 }}>
                    {[['Type',p.type],['Sum Insured',p.sumInsured],['Premium',`₹${p.premium?.toLocaleString()}/yr`],['Valid Until',formatDate(p.endDate)]].map(([k,v]) => (
                      <div key={k}>
