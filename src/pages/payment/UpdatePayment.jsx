...
const MOCK = [
  {
    id: 'PAY-2026-001', customerName: 'Aarav Sharma',   policyNo: 'POL-2026-1001',
    product: 'Star Comprehensive Health',    method: 'Cheque',
  ...
  {
    id: 'PAY-2026-002', customerName: 'Priya Mehta',    policyNo: 'POL-2026-1002',
    product: 'HDFC ERGO Optima Secure',     method: 'NEFT',
  ...
  {
    id: 'PAY-2026-003', customerName: 'Rohan Verma',    policyNo: 'POL-2026-1003',
    product: 'Bajaj Allianz Health Guard',  method: 'Cheque',
  ...
  {
    id: 'PAY-2026-004', customerName: 'Sneha Iyer',     policyNo: 'POL-2026-1004',
    product: 'Star Comprehensive Health',   method: 'RTGS',
  ...
  {
    id: 'PAY-2026-005', customerName: 'Rahul Gupta',    policyNo: 'POL-2026-1005',
    product: 'Bajaj Allianz Comprehensive', method: 'IMPS',
  ...
  {
    id: 'PAY-2026-006', customerName: 'Kavita Pillai',  policyNo: 'POL-2026-1006',
    product: 'HDFC ERGO Optima Secure',     method: 'Cheque',
  ...
  {
    id: 'PAY-2026-007', customerName: 'Vikram Rao',     policyNo: 'POL-2026-1007',
    product: 'Star Comprehensive Health',   method: 'NEFT',
  ...
  {
    id: 'PAY-2026-008', customerName: 'Divya Nair',     policyNo: 'POL-2026-1008',
    product: 'Bajaj Allianz Health Guard',  method: 'IMPS',
  ...
  {
    id: 'PAY-2026-009', customerName: 'Arjun Singh',    policyNo: 'POL-2026-1009',
    product: 'HDFC ERGO Optima Secure',     method: 'Cheque',
  ...
  {
    id: 'PAY-2026-010', customerName: 'Meera Joshi',    policyNo: 'POL-2026-1010',
    product: 'Star Comprehensive Health',   method: 'RTGS',
...
function PaymentModal({ payment, onClose, onAccept, onReject }) {
  ...
  const confirmAccept = () => { onAccept(payment.id); setPhase('accepted') }
  const confirmReject = () => {
    if (!reason) return
  ...
  return (
    <>
      ...
      <div style={{
        ...
        boxShadow: '0 24px 60px rgba(0,0,0,.22)', width: '92%', maxWidth: 600,
        ...
        <div style={{ padding: '20px 26px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px 24px', marginBottom: 20 }}>
            ...
            {payment.branch    && <DetailRow label="Branch"          value={payment.branch} />}
            {payment.chequeNo  && <DetailRow label="Cheque Number"   value={payment.chequeNo} />}
            {payment.referenceNo && <DetailRow label="Reference / UTR" value={payment.referenceNo} />}
          </div>
          ...
          {payment.status === 'Rejected' && payment.rejectionReason && (
            <div style={{ background: '#fef2f2', border: '1.5px solid #fca5a5', borderRadius: 10, padding: '12px 16px', marginBottom: 18 }}>
              ...
              <div style={{ fontSize: 13.5, color: '#7f1d1d' }}>{payment.rejectionReason}</div>
            </div>
          )}

          {/* ── Success states ── */}
          {phase === 'accepted' && (
            <div style={{ background: '#f0fdf4', border: '1.5px solid #86efac', borderRadius: 12, padding: '18px 20px', display: 'flex', gap: 14, alignItems: 'flex-start' }}>
              <span style={{ fontSize: 26 }}>✅</span>
              <div>
                <div style={{ fontWeight: 700, color: '#15803d', fontSize: 15, marginBottom: 4 }}>Payment Accepted</div>
                <div style={{ fontSize: 13, color: '#166534' }}>
                  Payment of <strong>{fmt(payment.amount)}</strong> from <strong>{payment.customerName}</strong> has been successfully verified and accepted. Policy <strong>{payment.policyNo}</strong> will now be activated.
                </div>
              </div>
            </div>
          )}
          ...
          {phase === 'rejected' && (
            <div style={{ background: '#fef2f2', border: '1.5px solid #fca5a5', borderRadius: 12, padding: '18px 20px', display: 'flex', gap: 14, alignItems: 'flex-start' }}>
              <span style={{ fontSize: 26 }}>❌</span>
              <div>
                <div style={{ fontWeight: 700, color: '#dc2626', fontSize: 15, marginBottom: 4 }}>Payment Rejected</div>
                <div style={{ fontSize: 13, color: '#7f1d1d' }}>
                  Payment of <strong>{fmt(payment.amount)}</strong> has been rejected. Reason: <strong>{reason}</strong>. The customer will be notified to re-submit the payment.
                </div>
              </div>
            </div>
          )}
            ...
            <div style={{ background: '#fef9f0', border: '1.5px solid #fcd34d', borderRadius: 12, padding: '18px 20px' }}>
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                <button
                  ...
                  disabled={!reason}
                  style={{ padding: '9px 20px', borderRadius: 8, background: !reason ? '#e5e7eb' : '#dc2626', color: !reason ? '#9ca3af' : '#fff', border: 'none', fontWeight: 600, fontSize: 13.5, cursor: !reason ? 'not-allowed' : 'pointer', fontFamily: 'inherit' }}
                >
                  Confirm Rejection
                </button>
                <button type="button" className="btn btn-ghost" onClick={() => setPhase('view')}>Cancel</button>
              </div>
            </div>
          )}
          ...
          {phase === 'view' && canAct && (
            <div style={{ display: 'flex', gap: 10, paddingTop: 4 }}>
              <button
                ...
                style={{ flex: 1, padding: '11px 0', borderRadius: 9, background: '#16a34a', color: '#fff', border: 'none', fontWeight: 700, fontSize: 14, cursor: 'pointer', fontFamily: 'inherit', transition: 'opacity .13s' }}
              ...
              <button
                ...
                style={{ flex: 1, padding: '11px 0', borderRadius: 9, background: '#fff', color: '#dc2626', border: '2px solid #dc2626', fontWeight: 700, fontSize: 14, cursor: 'pointer', fontFamily: 'inherit', transition: 'all .13s' }}
        ...
        <div className="card-body">
          ...
          <div style={{ display: 'flex', gap: 6, marginBottom: 16, flexWrap: 'wrap' }}>
            {STATUS_TABS.map(t => (
              ...
              >
                {t} <span style={{ fontSize: 11, opacity: .7 }}>({counts[t]})</span>
              </button>
            ))}
          </div>
