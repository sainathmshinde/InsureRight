...
const BASE = {
  ...
  whatsappTemplateId: "", clickTracking: false, conversionTracking: false,
  status: "Active", isCampaignOpen: true, isActive: true,
  documentName: "", templateName: "", sentStatus: null,
  callingIds: [], salesIds: [],
};

const MOCK_DATA = {
  1:  { ...BASE, name: "Campaign 1",                                startDate: "2024-09-20", endDate: "2025-07-29", callingIds: [2],    salesIds: [1, 5] },
  5:  { ...BASE, name: "Campaign OPD and DIGIT PAYMENT PROTECTION", startDate: "2025-01-31", endDate: "2025-03-31", isCampaignOpen: false, callingIds: [4], salesIds: [1] },
  6:  { ...BASE, name: "BPP Campaign",                              startDate: "2025-02-19", endDate: "2025-03-24", documentName: "cyber_opd_new_design.pdf", templateName: "cyber_opd_new_design", callingIds: [2, 9], salesIds: [8] },
  7:  { ...BASE, name: "Test Campaign",                              startDate: "2025-08-03", endDate: "2025-08-29", sentStatus: 0, callingIds: [], salesIds: [1] },
  8:  { ...BASE, name: "SBI_STP_Campaign",                          startDate: "2025-09-18", endDate: "2026-03-10", sentStatus: 0, callingIds: [4], salesIds: [5] },
  11: { ...BASE, name: "BPP Campaign_2026-2027",                    startDate: "2026-03-16", endDate: "2026-05-31", sentStatus: 0, callingIds: [2, 9], salesIds: [8] },
  12: { ...BASE, name: "Standalone campaign",                        startDate: "2026-02-28", endDate: "2026-04-29", sentStatus: 0, callingIds: [2], salesIds: [1] },
};
...
const DEFAULT = {
  ...
  conversionTracking: false, status: "Active", callingIds: [], salesIds: [],
};
...
export default function CampaignEdit() {
  ...
  const toggleAssoc = (aid) =>
    setForm((prev) => ({
      ...prev,
      selectedAssociations: prev.selectedAssociations.includes(aid)
        ? prev.selectedAssociations.filter((x) => x !== aid)
  ...
  const toggleProduct = (pid) =>
    setForm((prev) => ({
      ...prev,
      selectedProducts: prev.selectedProducts.includes(pid)
        ? prev.selectedProducts.filter((x) => x !== pid)
        : [...prev.selectedProducts, pid],
    }));

  const handleSubmit = (e) => {
    e.preventDefault();
    console.log("Update Campaign:", id, form, { assignedCalling: [...assignedCalling], assignedSales: [...assignedSales] });
    navigate("/campaign");
  };
              ...
              <div className="form-grid-3">
                ...
                <Field label="Campaign Type" required>
                  <Select value={form.type} onChange={set("type")} required>
                    ...
                    {["Promotional","Incentive","Renewal","Awareness","Onboarding","Seasonal"].map((t) => (
                    ...
                    return (
                      <div
                        ...
                        style={{
                          ...
                          background: sel ? m.bg : "#fff", transition: "all .13s",
                    ...
                    <div style={{ marginTop: 16 }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                        ...
                        {selectedUserIds.size > 0 && (
                          <span style={{ fontSize: 13, color: "var(--brand-mid)", fontWeight: 600 }}>{selectedUserIds.size} selected</span>
                        )}
                      </div>
                      {filteredUsers.length === 0 ? (
                        <div style={{ textAlign: "center", padding: "32px 0", color: "var(--text-3)", fontSize: 14 }}>No users match the selected filters.</div>
                      ) : (
                        <>
                          <div className="table-wrap">
                            <table>
                              <thead>
                                <tr>
                                  <th style={{ width: 36 }}>
                                    <input type="checkbox" checked={allPageSelected} ref={(el) => { if (el) el.indeterminate = somePageSelected; }} onChange={togglePageAll} />
                                  </th>
                                  <th>#</th><th>Name</th><th>Type</th><th>Age</th><th>Gender</th><th>Marital Status</th><th>State</th><th>Segment</th>
                                </tr>
                              </thead>
                              <tbody>
                                {pagedUsers.map((u, i) => (
                                  <tr key={u.id} style={{ background: selectedUserIds.has(u.id) ? "var(--brand-light)" : undefined, cursor: "pointer" }} onClick={() => toggleUser(u.id)}>
                                    <td onClick={(e) => e.stopPropagation()}>
                                      <input type="checkbox" checked={selectedUserIds.has(u.id)} onChange={() => toggleUser(u.id)} />
                                    </td>
                                    <td>{(userPage - 1) * PAGE_SIZE + i + 1}</td>
                                    <td>{u.name}</td><td>{u.type}</td><td>{u.age}</td><td>{u.gender}</td><td>{u.marital}</td><td>{u.state}</td><td>{u.segment}</td>
                                  </tr>
                                ))}
                              </tbody>
                            </table>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
                ...
                <div>
                  <div style={{ fontSize: 13, fontWeight: 600, color: "var(--text-2)", marginBottom: 10, display: "flex", alignItems: "center", gap: 7 }}>
                    <span style={{ background: "#e0f2fe", color: "#0369a1", borderRadius: 6, padding: "2px 9px", fontSize: 12 }}>📞 Calling Employees</span>
                  </div>
                ...
                <div>
                  <div style={{ fontSize: 13, fontWeight: 600, color: "var(--text-2)", marginBottom: 10, display: "flex", alignItems: "center", gap: 7 }}>
                    <span style={{ background: "var(--brand-light)", color: "var(--brand)", borderRadius: 6, padding: "2px 9px", fontSize: 12 }}>💼 Sales Employees</span>
                  </div>
