...
export default function CustomerList() {
  ...
  const scopedData = user?.role === 'agent' ? customers.filter(c => c.agentId === user.id) : customers
    ...
    { key: 'actions',  label: 'Actions',
      render: row => (
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
          <Button variant="primary"   size="sm" onClick={() => navigate(`/policy/buy?customerId=${row.id}`)}>Buy Policy</Button>
...
const S = {
  pendingBtn: {
    background: '#fffbeb', border: '1.5px solid #fcd34d', borderRadius: 6,
